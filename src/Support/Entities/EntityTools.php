 ...
 * Tools module.
 *
 * This file is integrated into laravel-telegram-bot from the
 * danog/telegram-entities library (https://github.com/danog/telegram-entities)
 * to avoid an external dependency. It is adapted only by changing its namespace
 * and removing the webmozart/assert dependency; the original logic is preserved.
 *
...
namespace ItHealer\Telegram\Support\Entities;
...
final class EntityTools
{
    ...
    public static function mbStrSplit(string $text, int $length): array
    {
        ...
        foreach (\str_split($text, $length << 1) as $chunk) {
            ...
            if (! \is_string($chunk)) {
                throw new AssertionError('Expected a string after UTF-16 conversion.');
            }
            $result[] = $chunk;
        }
